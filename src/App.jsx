...
const tabs = [
  ...
  ['payments', 'Payments', '/payments', 'id', ['amountPaise']],
...
function Crud({ tab, rows, setRows, message, setMessage, load }) {
  ...
  async function submit(event) { event.preventDefault(); try { const payload = Object.fromEntries(fields.filter((field) => value(field) !== '').map((field) => [field, ['number', 'id', 'userId', 'tripId', 'coachId', 'booking', 'trainNumber', 'stationCode', 'fromSeq', 'toSeq', 'amountPaise', 'seq', 'age', 'seatNumber'].includes(field) ? Number(value(field)) : value(field)])); const result = await request(editing ? `${path}/${editing}` : path, { method: editing ? 'PUT' : 'POST', body: JSON.stringify(payload) }); if (key === 'coaches' && !editing && seatCount > 0) await request(`/coaches/${result.id}/seats/bulk?count=${seatCount}&berthType=${berthType}`, { method: 'POST' }); setRows((current) => editing ? current.map((row) => row[id] === editing ? result : row) : [result, ...current]); setEditing(null); setForm({}); setSeatCount(0); setMessage(key === 'coaches' && seatCount > 0 ? `${label} and ${seatCount} seats created` : `${label} saved`) } catch (error) { setMessage(error.message) } }
